        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-300">
                <div className="relative p-6 pt-8 bg-slate-50 border-b border-slate-100 flex flex-col items-center">
                    <button
                        onClick={onCancel}
                        className="absolute right-4 top-4 p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-200 rounded-full transition-all"
                    >
                        <X size={20} />
                    </button>
                    ...
                    <h3 className="text-xl font-black text-slate-800 uppercase tracking-tighter text-center">
                        Autenticação Necessária
                    </h3>
                    <p className="text-xs text-slate-500 font-bold uppercase tracking-widest text-center mt-1">
                        {waiter.name}
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-6">
                    <div className="text-center">
                        <p className="text-xs font-bold text-slate-600 uppercase">
                            Ação: <span className="text-blue-600">{actionDescription}</span>
                        </p>
                    </div>

                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Senha (PIN) do Garçom</label>
                        <input
                            type="password"
                            autoFocus
                            placeholder="Digite sua senha"
                            className="w-full text-center p-4 bg-slate-50 border-2 border-slate-200 rounded-2xl text-lg font-black outline-none focus:ring-4 focus:ring-blue-50 focus:border-blue-500 transition-all tracking-widest placeholder:tracking-normal"
                            value={password}
                    ...
                    {error && (
                        <div className="p-3 bg-red-50 text-red-600 rounded-xl text-xs font-bold uppercase tracking-widest text-center flex items-center justify-center gap-2">
                            <AlertCircle size={14} />
                            {error}
                        </div>
                    )}
